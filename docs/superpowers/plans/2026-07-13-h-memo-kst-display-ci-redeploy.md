...
- KST 수정 후 CI에서 발견된 후속 실패로, 현재 `firebase-tools`는 Java 21 이상을 요구하지만 CI가 Firestore 에뮬레이터 실행 전에 호환 JDK를 명시적으로 준비하지 않습니다.
...
5. CI에서 Firestore 보안 규칙 테스트 전에 Java 21을 명시적으로 설정합니다.
6. 관련 파일이 500줄을 넘기지 않도록 기존 작은 공용 모듈과 업데이트 데이터 파일만 수정합니다.
