...
GitHub Actions에서 내려받아 테스트할 때는 `h-memo-macos-dmg` 아티팩트를 우선 사용하세요. `.app` 폴더를 raw artifact로 직접 내려받으면 실행 권한이나 번들 메타데이터가 보존되지 않아 Finder에서 “이 Mac에서 지원되지 않음”처럼 보일 수 있습니다. 보관용 `.app`이 필요하면 `h-memo-macos-app-archive`의 `*_app.tar.gz`를 내려받아 압축을 해제하세요.

생성 위치:

- `apps/desktop/src-tauri/target/release/bundle/macos/*.app`
- `apps/desktop/src-tauri/target/release/bundle/macos/*_app.tar.gz`
...
그래도 열리지 않으면 앱을 `/Applications`로 복사한 뒤 터미널에서 quarantine 속성을 제거할 수 있습니다.

```bash
xattr -dr com.apple.quarantine "/Applications/H Memo.app"
open "/Applications/H Memo.app"
```
