...
`.env`, `.env.local`, mode별 env 파일 또는 CI에서 아래 값을 제공합니다. 이 저장소에는 실제 서비스 값이 포함되어 있지 않습니다.
...
설치된 앱이나 웹 미리보기에서 빌드 시점 환경 변수가 비어 있는 경우에는 앱 메뉴의 **구글 로그인 설정**에 같은 Firebase Web Client 값을 입력하고 **설정 저장**을 누릅니다. 저장된 값은 해당 기기의 브라우저/웹뷰 저장소에만 보관되며, **설정 지우기**로 삭제할 수 있습니다.
...
2. **Settings → Authorized domains**에는 scheme/port가 없는 host 또는 domain만 등록합니다.
   ...
   - Tauri 런타임: 로그인 실패 시 Firebase Auth 오류와 런타임 로그에 표시되는 origin/host를 확인한 뒤, 필요한 host만 등록합니다.
3. 웹 앱 구성의 API 키 / 프로젝트 ID / 앱 ID가 `.env`, CI 변수, 또는 앱 안의 **구글 로그인 설정** 값과 일치하는지 확인
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /users/{uid}/backupSnapshots/{snapshotId} {
      ...
      allow create, update: if isOwner(uid) && hasValidBackupSnapshotShape(uid);
