...
5. 데스크톱 Tauri 런타임에서는 시스템 기본 브라우저를 열고 Firebase Auth URI가 생성한 Google 로그인 결과를 로컬 loopback으로 받은 뒤 Firebase credential로 로그인합니다.
6. `VITE_GOOGLE_OAUTH_CLIENT_ID`는 선택입니다. Google Cloud Console → APIs & Services → Credentials에서 **Desktop app** 유형의 OAuth client를 만들어 주입하면 이 전용 OAuth 경로를 우선 사용합니다.
7. `VITE_GOOGLE_OAUTH_CLIENT_ID`가 없어도 기본 H Memo Firebase 프로젝트는 WebView 팝업/리다이렉트 대신 Firebase Auth URI 기반 시스템 브라우저 경로를 사용합니다.
