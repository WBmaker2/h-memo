...
5. 데스크톱 Tauri 런타임에서 팝업 로그인이 `auth/popup-blocked`로 차단되면 Firebase 리다이렉트 로그인으로 자동 전환합니다.
...
- OAuth 동작은 실제 Windows Tauri 런타임에서 최종 확인해야 합니다. 현재 앱은 팝업 차단 시 Firebase 리다이렉트 로그인으로 전환합니다. Google OAuth가 WebView 자체를 차단하는 환경이 확인되면 다음 단계로 시스템 브라우저 기반 로그인 흐름을 추가합니다.
