...
- 여러 메모를 만든 뒤 “서버 백업” 수행 후 결과에 따라 `새 백업을 저장했습니다.`, `오늘 백업을 최신 내용으로 교체했습니다.`, 또는 `변경된 내용이 없어 백업을 생략했습니다.`가 표시되는지 확인
- 메모를 수정/추가/삭제 후 “서버 복원”을 열어 날짜별 목록에서 특정 날짜를 선택하고, 선택한 백업의 메모만 반영되는지 확인
- 실패 시 화면 status 영역의 오류 메시지와 콘솔 로그 캡처

이 저장소의 unit test는 injectable Firestore driver로 실제 gateway의 chunk, activation, transaction, timestamp 정규화, wrapper 검증을 실행합니다. Firestore rules는 문자열 기반 policy regression test와 실제 Firebase Rules Emulator test를 모두 제공합니다. 로컬 Emulator 검증은 `npm run test:firestore-rules`로 실행하며, CI도 같은 명령을 실행합니다.
