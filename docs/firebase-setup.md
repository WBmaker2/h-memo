...
6. Google Cloud Console → APIs & Services → Credentials에서 **Desktop app** 유형의 OAuth client를 만들고 client ID를 `VITE_GOOGLE_OAUTH_CLIENT_ID`로 주입합니다.
7. `VITE_GOOGLE_OAUTH_CLIENT_ID`가 빠진 Windows 데스크톱 빌드는 Google 로그인 버튼을 비활성화하고 설정 필요 메시지를 표시합니다. 웹 OAuth client의 redirect URI는 loopback 랜덤 포트를 허용하지 않으므로 데스크톱 로그인 대체 경로로 사용하지 않습니다.
...
- OAuth 동작은 실제 Windows Tauri 런타임에서 최종 확인해야 합니다. 데스크톱 로그인은 `VITE_GOOGLE_OAUTH_CLIENT_ID`가 포함된 시스템 브라우저 OAuth 경로를 사용합니다.
