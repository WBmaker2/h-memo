...
- `VITE_GOOGLE_OAUTH_CLIENT_ID` (Windows/macOS/Linux 데스크톱 시스템 브라우저 로그인용, 선택)
...
5. 데스크톱 Tauri 런타임에서는 `VITE_GOOGLE_OAUTH_CLIENT_ID`가 있으면 시스템 기본 브라우저를 열고 loopback redirect로 Google OAuth 결과를 받은 뒤 Firebase credential로 로그인합니다.
6. `VITE_GOOGLE_OAUTH_CLIENT_ID`는 Google Cloud Console → APIs & Services → Credentials에서 **Desktop app** 유형의 OAuth client를 만들어 사용합니다. 같은 Firebase/Google Cloud 프로젝트의 Google provider가 활성화되어 있어야 합니다.
7. 이 값이 없으면 기존 Firebase Web SDK 팝업/리다이렉트 경로를 시도하지만, Google 정책과 WebView 제한 때문에 실제 Windows WebView에서는 차단될 수 있습니다.
...
- OAuth 동작은 실제 Windows Tauri 런타임에서 최종 확인해야 합니다. 안정적인 데스크톱 로그인은 `VITE_GOOGLE_OAUTH_CLIENT_ID`가 포함된 시스템 브라우저 OAuth 경로를 사용합니다.
