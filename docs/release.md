...
`windows-tauri.yml` 워크플로우는 기존처럼 PR/브랜치 빌드에서는 MSI/NSIS 아티팩트를 업로드하고, 태그 기반 또는 수동 실행 시 GitHub Release까지 자동 업로드합니다.
...
2. **Run workflow** → `release_tag` 입력 (예: `v0.1.0`)
3. 실행하면 현재 커밋 기준으로 릴리스 아티팩트가 업로드됩니다.
